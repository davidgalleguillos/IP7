...
class DiscoveryService:
    ...
    def __init__(self, ipv7_address: str, udp_tunnel_port: int, on_node_discovered: Optional[Callable[[str, str, int], Any]] = None):
    ...
    async def _broadcast(self):
        ...
        while self._running:
            try:
                ...
                await loop.run_in_executor(None, sock.sendto, message, ('<broadcast>', self.DISCOVERY_PORT))
            ...
            try:
                ...
                if node_v7 and node_v7 != self.ipv7_address:
                    if node_v7 not in self._discovered_nodes:
                        logging.info(f"Nuevo nodo IPv7 descubierto: {node_v7} en {node_ip}:{node_port}")
                        ...
                        self._discovered_nodes[node_v7]["last_seen"] = asyncio.get_event_loop().time()
