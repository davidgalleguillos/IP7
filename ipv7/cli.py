...
from ipv7 import (
    IPv7Header, IPv7Router, QoSLevel, AIRouter, 
    NetworkState, QuantumInterface, SmartFirewall
)
...
async def cmd_route(args):
    ...
    if args.route_action == 'list':
        ...
        for prefix, entry in state["routing_table"].items():
            print(f"  Prefix: {prefix.hex()[:16]}... -> Next Hop: {entry.next_hop.hex()[:16]}... [{entry.interface}] Metric: {entry.metric}")
        ...
        ns = NetworkState(args.latency, args.bandwidth, args.congestion, args.error_rate, args.quantum, QoSLevel.BEST_EFFORT)
        ai = AIRouter()
...
def main():
    ...
    send_p.add_argument('--qos', choices=[e.name for e in QoSLevel], default='BEST_EFFORT')
